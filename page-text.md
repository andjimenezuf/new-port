# Page Text Export

This file captures the user-visible text from the page components and buttons, excluding company images and logos.

## Head

- Andres Jimenez

## Navbar

- Andres Jimenez
- Software Engineer
- Home
- About
- Experience
- Projects

## Hero Section

- Miami to Gainesville
- Hello, I'm Andres.
- I build software across AI, data, and product engineering, with a portfolio focused on ambitious student work, internships, and fast-moving technical experiments.

### Hero Buttons / Links

- About Me
- Experience
- Projects
- Awards

### Hero Info

- Current focus
- Software engineering, ML systems, cloud tooling
- Based in
- Florida, United States
- Building with
- Next.js, Python, AI APIs

## About Section

- About Me
- A first-generation University of Florida student aiming at technical work that blends rigorous engineering with fast iteration.
- What I'm Studying
- B.S. in Computer Science at the University of Florida
- Minor in Statistics
- Focused on building a strong base in systems, data, and applied machine learning
- What I'm Into
- I'm most interested in software engineering, cloud systems and scalable infrastructure, and AI/ML. I tend to learn best by building real things, testing them under pressure, and refining them through iteration.
- Outside the Classroom
- Music
- Cycling
- Staying curious and building taste through side projects and hobbies

### Technical Skills

- Technical Skills
- Programming Languages
- Python
- JavaScript
- TypeScript
- C++
- Java
- Swift
- HTML
- CSS
- SQL
- R
- Frameworks & Tools
- React
- Next.js
- FastAPI
- Node.js
- Postman
- LangGraph
- PyTorch
- TensorFlow
- TensorRT
- Ollama
- LLMs
- AWS
- Docker

### Certifications

- Certifications
- AWS Cloud Practitioner

## Education Section

- Education
- University of Florida
- Bachelor of Science in Computer Science, Minor in Statistics
- Gainesville, FL
- Expected Dec 2026

### Relevant Coursework

- Relevant Coursework
- Data Structures & Algorithms
- Object-Oriented Programming
- Intro to Machine Learning
- Intro to Software Engineering
- Operating Systems
- Computer Networking
- Natural Language Processing

### Clubs & Affiliations

- Clubs & Affiliations
- AISES
- Advancing Indigenous People in STEM
- ColorStack UF
- Diversity in Computing
- SHPE
- Society of Hispanic Professional Engineers
- SEO Career
- Career Prep Fellow

## Experience Section

- Experience

### Experience Item 1

- Software Engineer Intern
- JPMorgan Chase & Co.
- New York City, NY
- June 2025 - August 2025
- Integrated AI agents into JPMC's investment platform to streamline risk analysis, cutting review time by roughly 40%.

### Experience Item 2

- AI Software Engineer Intern
- Advantech
- Tokyo, Japan
- February 2025 - May 2025
- Built a real-time computer vision system using NVIDIA tooling and architected a multilingual AI assistant pipeline later showcased at Japan IT Week.

## Leadership Section

- Leadership & Professional Development
- Peer Mentor - Miami Dade College Cloud Computing Center
- Miami, FL · Dec 2023 - Dec 2023
- Mentored 20 high school students through an AWS certification boot camp.
- Reached an 80% pass rate by combining hands-on labs with clear instruction.

## Awards Section

- Awards & Scholarships

### Award Item 1

- Hispanic Scholarship Fund Scholar
- Hispanic Scholarship Fund
- 2024
- Recognized for academic excellence and leadership within the Hispanic community.

### Award Item 2

- Palantir Future Scholar
- Palantir Technologies
- 2023
- Selected for Palantir's scholar program supporting students entering technology and data work.

### Award Item 3

- The Machen Florida Opportunity Scholar
- University of Florida
- 2022
- Awarded a major scholarship for first-generation students at the University of Florida.

### Award Item 4

- MLT Career Prep Scholar
- Management Leadership for Tomorrow
- 2027 Cohort
- Part of the MLT Career Prep cohort focused on software engineering, financial services, and leadership development.

## Projects Section

- Projects
- A mix of hackathon work, personal products, and experiments across AI, mobile, and full-stack development.

### Project 1

- FairFlow - HackPrinceton
- November 2025
- Built a community micro-lending platform that explains credit risk with AI-generated clarity scores and public accountability for merchants.
- FastAPI
- React
- xAI API
- Kotlin
- Visual coming soon

### Project 2

- Sherpa - HackHarvard
- October 2025
- Improved web accessibility for visually-impaired users by generating Gemini-based summaries of complex webpages and reducing DOM analysis latency.
- JavaScript
- FastAPI
- HTML
- CSS
- Google APIs
- Visual coming soon

### Project 3

- EduAdvisor - Personal Project
- September 2025
- Built an AI advisor that analyzes majors and prerequisite paths, then creates personalized recommendations using agent-driven tools.
- React
- TypeScript
- FastAPI
- MongoDB
- Google ADK
- Visual coming soon

### Project 4

- Currency Convertor
- Developed an iOS app in Swift for real-time currency conversions using ExchangeRateAPI and FlagsAPI.

### Project 5

- Ace Expense
- Streamlined personal finance management using shadcn/ui, Drizzle ORM, PostgreSQL, and Next.js.

### Project 6

- Investi Track
- Implemented a stock search and visualization interface using Next.js and AlphaVantage with room for predictive ML additions.

### Project 7

- Fruit Image Mold Detector
- Leveraged TensorFlow and a pre-trained ImageNet model to identify mold on fruits with 97% validation accuracy.

### Project 8

- Musicales Jimenez
- An interactive multi-page music store simulation with live cart calculations and SQL-backed inventory management.

## Footer

- © 2026 Andres Jimenez
- Resume
- Copied
